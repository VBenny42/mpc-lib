...
class setup_persistency : public fireblocks::common::cosigner::
...
private:
  ...
  const std::string
  get_tenantid_from_keyid(const std::string &key_id) const override;
  void
  load_key_metadata(const std::string &key_id,
  ...
  void store_key_metadata(
      const std::string &key_id,
      const fireblocks::common::cosigner::cmp_key_metadata &metadata) override;
  ...
  void store_setup_data(
      const std::string &key_id,
      const fireblocks::common::cosigner::setup_data &metadata) override;
  void
  load_setup_data(const std::string &key_id,
                  fireblocks::common::cosigner::setup_data &metadata) override;
  void store_setup_commitments(
      const std::string &key_id,
      const std::map<uint64_t, fireblocks::common::cosigner::commitment>
  ...
  void delete_temporary_key_data(const std::string &key_id,
                                 bool delete_key = false) override;
  ...
  std::map<std::string,
           std::map<uint64_t, fireblocks::common::cosigner::commitment>>
      _commitments;
};
...
int my_ed25519_verify(const struct ed25519_algebra_ctx *ed25519,
