    ...
    namespace common
    {
        namespace cosigner
        {
            ...
            void cmp_setup_service::create_secret(const std::string &key_id, const std::map<uint64_t, std::map<uint64_t, byte_vector_t>> &paillier_large_factor_proofs, std::string &public_key, cosigner_sign_algorithm &algorithm)
            {
                ...
                uint64_t player_id = _service.get_id_from_keyid(key_id);

                LOG_INFO("\nbackuping keyid %s for player %lu...", key_id.c_str(), player_id);
                ...
                algorithm = algo;
                LOG_INFO("key share created for: \nplayerId = %lu\nkeyid = %s\nalgorithm = %s\n", player_id, key_id.c_str(), to_string(metadata.algorithm));
            }
            ...
            elliptic_curve256_algebra_ctx_t *cmp_setup_service::get_algebra(cosigner_sign_algorithm algorithm)
            {
                ...
                {
                case ECDSA_SECP256K1:
                    return _secp256k1.get();
                case ECDSA_SECP256R1:
                    return _secp256r1.get();
                case EDDSA_ED25519:
                    return _ed25519.get();
                case ECDSA_STARK:
                    return _stark.get();
